...
export default function VenueMenuPage() {
  ...
  usePageTitle(venue ? `منوی ${venue.name}` : 'منوی کافه');
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== 'CUSTOMER') {
      guestName = window.prompt('برای پیوستن به سفارش گروهی، نام خود را وارد کنید:', 'مهمان');
  ...
  async function buyPunchCard(planId) {
    try {
      ...
      toast.success('بسته با موفقیت خریداری شد.');
      refreshPunchCards();
    } catch (err) {
      toast.error(err.response?.data?.message || 'خرید بسته ناموفق بود.');
    }
  }
  ...
  const menuSections = useMemo(() => {
    const secs = [];
    if (combos.length > 0) secs.push({ id: 'combos', name: 'کمبوهای ویژه', count: combos.length });
  ...
  async function callWaiter() {
    ...
    try {
      ...
      toast.success('گارسون خبر شد و به‌زودی به میز شما می‌آید.');
    } catch (err) {
      toast.error(err.response?.data?.message || 'ارسال درخواست با خطا مواجه شد.');
    } finally {
  ...
  async function startGroupOrder() {
    ...
    try {
      let guestName;
      if (!user || user.role !== 'CUSTOMER') {
        guestName = window.prompt('نام خود را برای شروع سفارش گروهی وارد کنید:', 'مهمان');
        if (!guestName) return;
      }
  ...
  function requestCheckout() {
    if (venue?.isTemporarilyClosed) {
      toast.error(venue.temporarilyClosedReason || 'این مجموعه امروز موقتاً سفارش نمی‌پذیرد.');
      return;
    }
    if (!tableId) {
      setTableInput('');
  ...
  async function resolveTableAndCheckout() {
    ...
    if (!num) { setTableError('شماره میز را وارد کنید.'); return; }
    ...
    try {
      ...
      setTableError(err.response?.status === 404 ? `میز شماره ${num} در این کافه پیدا نشد.` : 'خطا در بررسی شماره میز.');
    } finally {
  ...
  async function handleCheckoutWithTable(resolvedTableId, isPickup = false) {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'ثبت سفارش با خطا مواجه شد.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function payBillShare(share) {
    try {
      ...
      toast.error(err.response?.data?.message || 'خطا در پرداخت سهم');
    }
  }
      ...
      <Card className="max-w-md mx-auto text-center space-y-4">
        <h2 className="text-lg font-bold text-ink">
          <span className="flex items-center justify-center gap-2">
            ...
            {success ? 'پرداخت با موفقیت انجام شد' : 'پرداخت ناموفق یا در انتظار تایید'}
          </span>
        </h2>
        <p className="text-gray-600">شماره سفارش: {result.order.id}</p>
    ...
    return (
      <div className="space-y-4 py-4 animate-fade-up">
        ...
        <p className="text-center text-ink/40 text-sm pt-2">
          {joiningSession ? 'در حال پیوستن به سفارش گروهی...' : 'در حال بارگذاری منو...'}
        </p>
      </div>
    );
  }
    ...
    <div className="animate-fade-up">
        ...
        <div>
          ...
          {tableId && (
            <span className="inline-flex items-center gap-1.5 bg-green-50 border border-green-200 text-green-700 text-xs font-bold px-2.5 py-1 rounded-full mt-2">
              <CheckCircle size={12} />
              میز شما ثبت شد — سفارش به همین میز می‌رسد
            </span>
          )}
        </div>
        {user?.role === 'CUSTOMER' && (
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        {tableId && (
          ...
          >
            ...
            {waiterCalledAt && Date.now() - waiterCalledAt < 60000 ? 'گارسون خبر شد' : 'صدا زدن گارسون'}
          </button>
        )}
      </div>
      ...
      {happyHour && !venue.isTemporarilyClosed && (
        <div className="mb-4 rounded-2xl bg-gradient-to-l from-accent-500 to-accent-600 text-white px-4 py-3 flex items-center gap-3 shadow-accent-glow animate-fade-up">
          ...
          <div>
            <p className="font-extrabold text-sm">ساعت شاد فعال است!</p>
            <p className="text-xs text-white/85 mt-0.5">
              {Number(happyHour.discountPercent).toLocaleString('fa-IR')}٪ تخفیف خودکار روی این سفارش اعمال می‌شود.
            </p>
          </div>
        </div>
      )}
      ...
      {!!venue.isTemporarilyClosed && (
        <Card className="mb-4 border-red-200 bg-red-50/60">
          <p className="text-sm font-bold text-red-700">این مجموعه امروز موقتاً تعطیل است</p>
          <p className="text-xs text-red-600/80 mt-1">
            {venue.temporarilyClosedReason || 'در حال حاضر امکان ثبت سفارش جدید وجود ندارد.'}
          </p>
        </Card>
      )}
          ...
          <div className="flex gap-2.5 overflow-x-auto no-scrollbar pb-1">
            {recommendations.map((item) => (
              <div key={item.id} className="glass rounded-2xl p-3 shrink-0 w-40">
                ...
                <p className="text-xs text-ink/50 mt-0.5">{Number(item.price).toLocaleString('fa-IR')} تومان</p>
                <button
                  type="button"
      ...
      {lastOrder && !sessionId && cart.length === 0 && (
        <Card className="mb-4 flex items-center justify-between gap-3 border-accent-200/60 bg-accent-50/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-ink">دوباره همون همیشگی؟</p>
              <p className="text-xs text-ink/50 mt-0.5">سفارش آخرت رو با یک لمس تکرار کن.</p>
            </div>
          </div>
          <Button variant="secondary" onClick={reorderLast} disabled={reordering} className="shrink-0">
            {reordering ? 'در حال بارگذاری…' : 'سفارش مجدد'}
          </Button>
        </Card>
      )}
      ...
      {savedCartItems && !savedCartBannerDismissed && cart.length === 0 && (
        <Card className="mb-4 flex items-center justify-between gap-3 bg-primary-50 border-primary-200">
          <p className="text-sm text-gray-700">سبد ذخیره‌شده از آخرین بازدید شما موجود است.</p>
          <div className="flex gap-2 flex-shrink-0">
            <Button variant="secondary" onClick={loadSavedCart}>بارگذاری</Button>
            <Button variant="ghost" onClick={() => { setSavedCartBannerDismissed(true); setSavedCartItems(null); }}>نادیده گرفتن</Button>
          </div>
        </Card>
      )}
      ...
      {tableId && !sessionId && flags.groupOrdering && (
        <Card className="mb-6 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <p className="font-semibold text-ink text-sm">سفارش گروهی روی این میز</p>
            <p className="text-xs text-ink/50 mt-1">با دوستانت یک سبد سفارش مشترک بساز و همزمان اضافه کنید.</p>
          </div>
          <Button variant="secondary" onClick={startGroupOrder} disabled={startingSession}>
            {startingSession ? 'در حال ایجاد...' : 'شروع سفارش گروهی'}
          </Button>
        </Card>
      )}
      ...
      {sessionId && session && (
        <Card className="mb-6 space-y-3">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <p className="font-semibold text-ink text-sm">سفارش گروهی فعال</p>
              <p className="text-xs text-ink/50 mt-1">
                {session.participants.map((p) => p.label).join('، ')} — این لینک یا کد QR را برای دوستانت بفرست
              </p>
            </div>
            <img
              src={`${FILE_BASE_URL}/api/table-sessions/${sessionId}/qrcode.png`}
              alt="کد QR سفارش گروهی"
      ...
      {menuQuery || customizableOnly || maxPrice < priceCeiling ? (
        filteredItems.length === 0 && (
          <p className="text-center text-ink/40 text-sm py-10">موردی با این فیلترها پیدا نشد.</p>
        )
      ) : null}
            ...
            {combos.map((combo) => (
              <Card key={combo.id} className="flex items-center justify-between gap-4 border-accent-200/60 bg-accent-50/40">
                <div>
                  ...
                  <p className="text-accent-700 font-extrabold mt-2">{Number(combo.price).toLocaleString('fa-IR')} <span className="text-xs font-medium text-ink/40">تومان</span></p>
                </div>
                <Button variant="accent" onClick={() => addComboToCart(combo)} className="shrink-0">افزودن</Button>
              </Card>
            ))}
          </div>
        </section>
        ...
        <section key={category.id} id={`menu-sec-${category.id}`} className="mb-6 scroll-mt-32">
          <h2 className="flex items-center gap-2 font-extrabold text-ink mb-3">
            ...
            <span className="text-xs font-medium text-ink/35">
              {(itemsByCategory.get(category.id) || []).length.toLocaleString('fa-IR')} مورد
            </span>
          </h2>
          <div className="grid sm:grid-cols-2 gap-3">
      ...
      <CartDrawer
        ...
        checkoutLabel={sessionId ? 'ثبت و پرداخت سفارش گروهی' : undefined}
      >
        ...
        {!sessionId && (
          <div className="space-y-3 mb-3">
            ...
            {user?.role === 'CUSTOMER' && loyaltyBalance > 0 && (
              <div>
                <label className="text-xs text-gray-600">
                  استفاده از امتیاز وفاداری (موجودی: {loyaltyBalance} امتیاز، هر امتیاز ۱٬۰۰۰ تومان)
                </label>
                <input
                  type="number"
                  min={0}
            ...
            {user?.role === 'CUSTOMER' && walletBalance > 0 && (
              <div>
                <label className="text-xs text-gray-600">
                  پرداخت از کیف پول (موجودی: {Number(walletBalance).toLocaleString('fa-IR')} تومان)
                </label>
                <input
                  type="number"
                  min={0}
                ...
                {walletAmount > 0 && (
                  <p className="text-xs text-green-600 mt-1">
                    {Number(walletAmount).toLocaleString('fa-IR')} تومان از کیف پول — مابقی از درگاه: {Math.max(0, subtotal - totalDiscount - walletAmount).toLocaleString('fa-IR')} تومان
                  </p>
                )}
              </div>
            )}
            ...
            {paymentMethods.length > 1 && Math.max(0, subtotal - totalDiscount - walletAmount) > 0 && (
              <div>
                <label className="text-xs text-gray-600">روش پرداخت</label>
            ...
            {user?.role === 'CUSTOMER' && estimatedLoyaltyPoints > 0 && (
              <p className="flex items-center gap-1.5 text-xs font-bold text-accent-800 bg-accent-50 border border-accent-200/70 rounded-xl px-3 py-2">
                <Gift size={13} />
                با این سفارش {estimatedLoyaltyPoints.toLocaleString('fa-IR')} امتیاز وفاداری می‌گیری
              </p>
            )}
          </div>
        )}
      ...
      {punchCardPlans.length > 0 && (
        <div className="mt-6">
          <h3 className="font-bold text-ink mb-3">بسته‌های پیش‌پرداخت</h3>
          <div className="grid sm:grid-cols-2 gap-3">
            {punchCardPlans.map((plan) => (
              <Card key={plan.id} className="flex items-center justify-between">
                ...
                {user?.role === 'CUSTOMER' && (
                  <Button onClick={() => buyPunchCard(plan.id)}>خرید</Button>
                )}
              </Card>
            ))}
          </div>
      ...
      {showTableModal && (
        ...
        >
          <div className="glass-strong rounded-3xl shadow-lift w-full max-w-xs p-6 space-y-4 animate-pop-in" dir="rtl">
            <h3 className="font-black text-ink text-lg">شماره میز خود را وارد کنید</h3>
            <p className="text-sm text-ink/50">شماره‌ای که روی میز نوشته شده را وارد کنید.</p>
            <input
              type="text"
              inputMode="numeric"
            ...
            >
              {tableResolving ? 'در حال بررسی...' : 'تأیید و ثبت سفارش'}
            </button>
            ...
            {!!venue.acceptsPickup && (
              <>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] text-ink/40">یا</span>
                  <span className="flex-1 h-px bg-black/10" />
                </div>
                <button
                  type="button"
