...
export default function Login() {
  ...
  const submitPassword = (e) => {
    e.preventDefault();
    if (captcha.enabled && !captcha.token) return setError(t('auth.captchaRequired'));
    wrap(async () => done(await login(form.email, form.password, captcha.token)));
  };
  ...
  const sendOtp = (e) => {
    ...
    if (captcha.enabled && !captcha.token) return setError(t('auth.captchaRequired'));
    wrap(async () => {
      await requestLoginOtp(phone, captcha.token);
      setOtpStage('code');
    });
  };
  ...
  if (challengeToken) {
    return (
      <div className="max-w-sm mx-auto pt-6 animate-fade-up">
        <div className="text-center mb-6">
          <span className="inline-flex w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-700 to-primary-900 text-accent-300 items-center justify-center shadow-button mb-3">
            <ShieldCheck size={26} />
          </span>
          <h2 className="text-2xl font-black text-ink">{t('auth.twoFactorTitle')}</h2>
          <p className="text-sm text-ink/50 mt-1">{t('auth.twoFactorSubtitle')}</p>
        </div>
  ...
  return (
    <div className="max-w-sm mx-auto pt-6 animate-fade-up">
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl font-black text-ink">{t('auth.welcomeBack')}</h2>
        <p className="text-sm text-ink/50 mt-1">{t('auth.loginSubtitle')}</p>
      </div>
