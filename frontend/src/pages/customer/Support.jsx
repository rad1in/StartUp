...
const DEPARTMENTS = [
  { key: 'MANAGEMENT', label: 'مدیریت', icon: Building2 },
  { key: 'SALES', label: 'فروش', icon: BadgePercent },
  { key: 'TECHNICAL', label: 'فنی', icon: Wrench },
];
      ...
      {selectedId ? (
        ...
        <>
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-ink text-lg">پشتیبانی</h3>
            <Button onClick={() => setCreating(true)}>
              <Plus size={16} className="ml-1 inline" /> تیکت جدید
            </Button>
          </div>

          <div>
            <h4 className="font-semibold text-gray-700 mb-2 text-sm">تیکت‌های من</h4>
            {tickets.length === 0 && <p className="text-gray-500 text-sm">هنوز تیکتی ثبت نکرده‌اید.</p>}
            <div className="space-y-2">
              {tickets.map((t) => (
                <button key={t.id} onClick={() => setSelectedId(t.id)} className="w-full text-right">
                  <Card className="flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        {DEPARTMENTS.find((d) => d.key === t.department)?.label || t.department} ·{' '}
                        {new Date(t.createdAt).toLocaleDateString('fa-IR')}
                      </p>
                    </div>
                    <span
                      className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${
                        t.status === 'OPEN' ? 'border border-dashed border-gray-400 text-ink/60' : 'bg-primary-800 text-white'
                      }`}
                    >
                      {t.status === 'OPEN' ? <Clock size={11} /> : <CheckCircle size={11} />}
          ...
          <div>
            <h4 className="font-semibold text-gray-700 mb-2 text-sm">سوالات متداول</h4>
            <div className="space-y-2">
              {faqItems.map((faq) => (
...
function NewTicketPanel({ defaultOrderId, onClose, onCreated }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'ثبت تیکت ناموفق بود.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <Card className="border-2 border-primary-800">
      <h3 className="font-bold text-ink mb-4">تیکت جدید</h3>

      {!department ? (
        <div>
          <p className="text-sm text-gray-500 mb-3">این پیام مربوط به کدام واحد است؟</p>
        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-gray-500 mb-1">
            <span>واحد: {DEPARTMENTS.find((d) => d.key === department)?.label}</span>
          ...
          <textarea
            ...
            required
          />
          {defaultOrderId && <p className="text-xs text-gray-400">مرتبط با سفارش: {defaultOrderId}</p>}

          <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
            <Paperclip size={16} />
            ضمیمه فایل (عکس یا PDF، حداکثر ۳ فایل)
            <input
              type="file"
              multiple
              accept="image/*,.pdf"
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            ...
            این پیام محرمانه است (رمزنگاری‌شده ذخیره می‌شود)
          </label>
...
function TicketThread({ ticketId, onBack }) {
  ...
  if (!ticket) return <p className="text-gray-500 text-sm">در حال بارگذاری...</p>;
    ...
    <Card>
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            {DEPARTMENTS.find((d) => d.key === ticket.department)?.label} · {STATUS_LABEL[ticket.status]}
          </p>
        </div>
        {ticket.status === 'OPEN' && (
