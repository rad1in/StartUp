...
export default function SmsCampaigns() {
  ...
  async function reject(id) {
    const reason = window.prompt('دلیل رد کمپین (اختیاری):') || '';
    setBusyId(id);
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      {campaigns.length === 0 && <p className="text-sm text-gray-400">کمپینی یافت نشد.</p>}
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {campaigns.map((c) => (
          <Card key={c.id}>
            ...
            <p className="text-xs text-gray-500 mt-1">مجموعه: {c.venueName}</p>
            ...
            {c.status === 'PENDING' && (
              <div className="flex gap-2 mt-3">
                <Button onClick={() => approve(c.id)} disabled={busyId === c.id}>
                  <span className="flex items-center gap-1"><Check size={14} />تایید و ارسال</span>
                </Button>
                <Button variant="danger" onClick={() => reject(c.id)} disabled={busyId === c.id}>
                  <span className="flex items-center gap-1"><X size={14} />رد کردن</span>
                </Button>
              </div>
            )}
            {c.status === 'REJECTED' && c.rejectionReason && (
              <p className="text-xs text-red-600 mt-2">دلیل رد: {c.rejectionReason}</p>
            )}
          </Card>
        ))}
      </div>
