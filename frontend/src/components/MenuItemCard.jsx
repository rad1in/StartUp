      ...
      <Card className={`flex items-center justify-between gap-4 hover:shadow-lift transition-shadow ${!item.isAvailable ? 'opacity-60' : ''}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2 mt-2">
            <p className="text-accent-700 font-extrabold">{Number(item.price).toLocaleString('fa-IR')} <span className="text-xs font-medium text-ink/40">تومان</span></p>
            {hasModifiers && (
        ...
        <Button disabled={!item.isAvailable} onClick={handleAdd} className="shrink-0">
          {item.isAvailable ? 'افزودن +' : 'ناموجود'}
        </Button>
      </Card>
