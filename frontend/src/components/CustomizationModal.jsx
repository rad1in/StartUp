    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-xl p-5 space-y-5 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 p-1 rounded" aria-label="بستن">
            <X size={18} />
          </button>
        </div>
        ...
        {modifierGroups.map((group) => (
          <div key={group.id}>
            <div className="flex items-center gap-2 mb-2">
              ...
              {!!group.isRequired && (
                <span className="text-xs text-ink bg-ink/10 px-1.5 py-0.5 rounded">الزامی</span>
              )}
              {group.maxSelections !== null && group.maxSelections !== undefined && (
                <span className="text-xs text-gray-400">حداکثر {group.maxSelections}</span>
              )}
            </div>
                  ...
                  >
                    ...
                    {Number(opt.priceAdjustment) !== 0 && (
                      <span className="text-xs text-primary-700 font-medium">
                        +{Number(opt.priceAdjustment).toLocaleString('fa-IR')} تومان
                      </span>
                    )}
                  </label>
                ))}
        ...
        <div className="pt-3 border-t border-gray-100 space-y-3">
          {priceAdjustment !== 0 && (
            <p className="text-sm text-gray-600">
              افزایش قیمت: <span className="font-bold text-primary-700">+{priceAdjustment.toLocaleString('fa-IR')} تومان</span>
            </p>
          )}
          <p className="text-sm font-bold text-gray-800">
            قیمت نهایی: {(Number(item.price) + priceAdjustment).toLocaleString('fa-IR')} تومان
          </p>
