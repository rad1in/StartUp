...
const STEPS = [
  { key: 'PENDING', label: 'ثبت سفارش', icon: Receipt },
  { key: 'PREPARING', label: 'در حال آماده‌سازی', icon: ChefHat },
  { key: 'READY', label: 'آماده سرو', icon: Bell },
  { key: 'SERVED', label: 'سرو شده', icon: Handshake },
];
  ...
  if (order.status === 'CANCELLED') {
    return (
      <div className="card-luxe p-4 flex items-center gap-3 mb-4">
        ...
        <div>
          <p className="font-bold text-ink text-sm">سفارش لغو شد</p>
  ...
  return (
    <div className="card-luxe p-5 mb-4">
      <div className="flex items-center justify-between mb-5">
        <p className="font-extrabold text-ink text-sm">وضعیت سفارش</p>
        <span className="inline-flex items-center gap-1 text-xs text-ink/45">
          <Clock size={12} />
          {elapsed.toLocaleString('fa-IR')} دقیقه پیش ثبت شد
        </span>
      </div>
      ...
      {order.status !== 'SERVED' && (
        <p className="text-center text-[11px] text-ink/40 mt-4">
          {order.prepEstimate
            ? `تخمین آماده‌سازی: حدود ${order.prepEstimate.estimatedMinutes.toLocaleString('fa-IR')} دقیقه دیگر${
                order.prepEstimate.basedOnHistory ? '' : ' (تخمین اولیه)'
              }`
            : 'معمولاً آماده‌سازی حدود ۱۰ تا ۱۵ دقیقه طول می‌کشد'}
        </p>
      )}
    </div>
  );
