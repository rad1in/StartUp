...
export function AuthProvider({ children }) {
  ...
  const login = useCallback(
    async (email, password, captchaToken) =>
      applyAuth((await api.post('/auth/login', { email, password, captchaToken })).data),
    [applyAuth]
  );

  const requestLoginOtp = useCallback(
    (phone, captchaToken) => api.post('/auth/login/otp/request', { phone, captchaToken }).then((r) => r.data),
    []
  );
