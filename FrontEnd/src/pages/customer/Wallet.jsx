...
export default function Wallet() {
  ...
  async function handleTopUp(e) {
    ...
    try {
      ...
      if (data.success) {
        setMsg({ type: 'success', text: `${t('wallet.topUpSuccessPrefix')} ${n(Number(data.balance))} ${t('menu.toman')}` });
        setTopUpAmount('');
        await refresh();
              ...
              <div key={tx.id} className="py-3 flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className={`font-semibold text-sm ${TX_TYPE_COLOR[tx.type] || 'text-gray-600'}`}>
                    {tx.type === 'SPEND' ? '-' : '+'}{n(Number(tx.amount))} {t('menu.toman')}
                  </p>
                  <p className="text-xs text-gray-400">
                    {t('wallet.balanceAfter')} {n(Number(tx.balanceAfter))}
                  </p>
                </div>
              </div>
            ))}
