            ...
            <div key={tier.id} className="py-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {t('admin.gamification.minSpendLabel')}: {n(Number(tier.minSpend))} — {t('admin.gamification.minOrdersLabel')}: {tier.minOrders} — {t('admin.gamification.pointsMultiplierLabel')}: {tier.pointsMultiplier} {t('admin.gamification.timesSuffix')}
                </p>
              </div>
          ...
          {badges.map((badge) => (
            <div key={badge.id} className="py-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {CRITERIA_LABELS[badge.criteriaType]} ≥ {n(Number(badge.threshold))} — {badge.description}
                </p>
              </div>
              <div className="flex gap-2">
