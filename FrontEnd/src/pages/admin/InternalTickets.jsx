...
export default function InternalTickets() {
  const { t, n, date, dateShort } = useLanguage();
    ...
    <div className="space-y-5">
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Card><p className="text-xs text-gray-500">{t('admin.internalTickets.totalTickets')}</p><p className="text-xl font-bold">{n(Number(summary.total))}</p></Card>
          <Card><p className="text-xs text-gray-500">{t('admin.internalTickets.statusOpen')}</p><p className="text-xl font-bold text-red-600">{n(Number(summary.open))}</p></Card>
          <Card><p className="text-xs text-gray-500">{t('admin.internalTickets.statusInProgress')}</p><p className="text-xl font-bold text-yellow-600">{n(Number(summary.inProgress))}</p></Card>
            ...
            {tickets.map((tk) => (
              ...
              >
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {tk.assignedToName ? <span className="flex items-center gap-1"><ArrowRight size={12} />{tk.assignedToName}</span> : t('admin.internalTickets.noAssignment')} — {dateShort(tk.createdAt)}
                </p>
              </button>
            ))}
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {t('admin.internalTickets.createdLabel')}: {selected.createdByName} — {date(selected.createdAt)}
                ...
                <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
                  {(selected.comments || []).map((c) => (
                    <div key={c.id} className={`p-3 rounded-lg text-sm ${c.authorId === user?.id ? 'bg-primary-50 mr-6' : 'bg-gray-50 ml-6'}`}>
                      <p className="text-xs text-gray-400 mb-1">{c.authorName} ({c.authorRole}) — {date(c.createdAt)}</p>
