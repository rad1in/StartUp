      ...
      {summary && (
        <div className="grid sm:grid-cols-4 gap-3">
          ...
          <Card>
            ...
            <p className="text-xl font-bold text-gray-800">{n(summary.totalRevenue)} {t('common.toman')}</p>
          </Card>
          <Card>
            <p className="text-xs text-gray-500">{t('admin.venueDetail.platformCommission')}</p>
            <p className="text-xl font-bold text-gray-800">{n(summary.totalCommission)} {t('common.toman')}</p>
          </Card>
          <Card>
            <p className="text-xs text-gray-500">{t('admin.venueDetail.venueNetRevenue')}</p>
            <p className="text-xl font-bold text-primary-700">{n(summary.netRevenue)} {t('common.toman')}</p>
          </Card>
        </div>
      )}
        ...
        <div className="grid sm:grid-cols-2 gap-2">
          {venue.menuItems.map((item) => (
            <div key={item.id} className="flex items-center justify-between text-sm border-b border-gray-100 pb-1">
              <span>{item.name}</span>
              <span className="text-gray-500">{n(Number(item.price))} {t('common.toman')}</span>
            </div>
          ))}
