  ...
  return (
    <div className="animate-fade-up space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard icon={Users} label={t('admin.subscriptions.activeSubscribers')} value={n(stats.activeSubscribers)} />
        <StatCard icon={Wallet} label={t('admin.subscriptions.totalSubscriptionRevenue')} value={`${n(stats.totalRevenue)} ${t('admin.subscriptions.tomanAbbr')}`} />
        <StatCard icon={TrendingUp} label={t('admin.subscriptions.totalSalesCount')} value={n(stats.totalSubscriptionsSold)} />
        <StatCard
          icon={Percent}
          label={t('admin.subscriptions.averageDiscountGiven')}
          value={`${n(stats.averageDiscountPercent)}٪`}
        />
      </div>
              ...
              <tbody>
                {list.map((s) => (
                  <tr key={s.id} className="border-b border-ink/5 last:border-0">
                    ...
                    <td className="px-4 py-2.5 text-ink/70">{n(Number(s.pricePaid))} {t('admin.subscriptions.tomanAbbr')}</td>
