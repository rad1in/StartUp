...
function formatUptime(seconds, t, n) {
  const days = Math.floor(seconds / 86400);
  const hours = Math.floor((seconds % 86400) / 3600);
  const mins = Math.floor((seconds % 3600) / 60);
  if (days > 0) return `${n(days)} ${t('admin.liveOps.days')} ${n(hours)} ${t('admin.liveOps.hours')}`;
  if (hours > 0) return `${n(hours)} ${t('admin.liveOps.hours')} ${n(mins)} ${t('admin.liveOps.minutes')}`;
  return `${n(mins)} ${t('admin.liveOps.minutes')}`;
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <StatTile icon={AlertTriangle} label={t('admin.liveOps.apiErrorsLast24h')} value={health ? n(health.apiErrorsLast24h) : '—'} tone={errorTone} />
        <StatTile icon={CreditCard} label={t('admin.liveOps.failedPaymentRate24h')} value={`${failedPct}٪`} tone={failedTone} />
        <StatTile icon={Radio} label={t('admin.liveOps.liveConnections')} value={health ? n(health.connectedSockets) : '—'} />
        <StatTile icon={Clock} label={t('admin.liveOps.serverUptime')} value={health ? formatUptime(health.uptimeSeconds, t, n) : '—'} />
      </div>
