      ...
      <div className="flex items-center gap-2 mt-0.5">
        ...
        {hasChange && (
          <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded ${positive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
            {positive ? '▲' : '▼'} {Math.abs(changePct).toLocaleString(locale, { maximumFractionDigits: 1 })}٪
          </span>
        )}
      </div>
    </Card>
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className="border-b last:border-0 hover:bg-gray-50">
                    {Object.values(r).map((v, j) => <td key={j} className="py-1.5 pr-2 text-gray-700">{typeof v === 'number' ? n(Number(v)) : v != null ? String(v) : '—'}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeTab === 'overview' && overview && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <KPICard
              label={t('admin.bi.kpiGmv')} value={`${n(Number(overview.gmv))} ${t('common.toman')}`} color="text-primary-700"
              changePct={comparePrev && prevOverview ? pctChange(overview.gmv, prevOverview.gmv) : undefined}
            />
            <KPICard
              label={t('admin.bi.kpiCommissionRevenue')} value={`${n(Number(overview.commissionRevenue))} ${t('common.toman')}`} color="text-green-700"
              changePct={comparePrev && prevOverview ? pctChange(overview.commissionRevenue, prevOverview.commissionRevenue) : undefined}
            />
            <KPICard
              label={t('admin.bi.kpiTotalOrders')} value={n(Number(overview.totalOrders))}
              changePct={comparePrev && prevOverview ? pctChange(overview.totalOrders, prevOverview.totalOrders) : undefined}
            />
            <KPICard
              label={t('admin.bi.kpiAvgOrder')} value={`${n(Math.round(Number(overview.avgOrderValue)))} ${t('common.toman')}`}
              changePct={comparePrev && prevOverview ? pctChange(overview.avgOrderValue, prevOverview.avgOrderValue) : undefined}
            />
            <KPICard
              label={t('admin.bi.kpiActiveVenues')} value={n(Number(overview.activeVenues))}
              changePct={comparePrev && prevOverview ? pctChange(overview.activeVenues, prevOverview.activeVenues) : undefined}
            />
            <KPICard
              label={t('admin.bi.kpiUniqueCustomers')} value={n(Number(overview.uniqueCustomers))}
          ...
          {comparePrev && prevOverview && (
            <p className="text-[11px] text-gray-400">
              {t('admin.bi.comparePeriodLabel')} {dateShort(new Date(`${from}T00:00:00`) - 86400e3 - (new Date(`${to}T23:59:59`) - new Date(`${from}T00:00:00`)))}
              {' '}{t('admin.bi.dateRangeTo')}{' '}
              {dateShort(new Date(`${from}T00:00:00`) - 86400e3)}
            </p>
          )}
        </div>
      )}
              ...
              <tbody>
                {trend.map((r) => (
                  <tr key={r.period} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-1.5">{n(Number(r.orders))}</td>
                    <td className="py-1.5">{n(Number(r.gmv))}</td>
                    <td className="py-1.5 text-green-700">{n(Number(r.commission))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="space-y-2">
              {byTier.map((r) => (
                <div key={r.tier} className="flex items-center gap-3 py-2 border-b last:border-0">
                  ...
                  <span className="text-xs text-gray-500">{n(Number(r.orders))} {t('admin.bi.orderCountSuffix')}</span>
                  <span className="text-sm font-medium text-gray-900 mr-auto">{n(Number(r.gmv))} {t('admin.bi.gmvSuffix')}</span>
                  <span className="text-sm font-medium text-green-700">{n(Number(r.commission))} {t('admin.bi.commissionSuffix')}</span>
            ...
            <div className="space-y-2">
              {byCity.map((r) => (
                <div key={r.city} className="flex items-center gap-3 py-2 border-b last:border-0">
                  ...
                  <span className="text-xs text-gray-500">{r.venueCount} {t('admin.bi.venueCountSuffix')}</span>
                  <span className="text-xs text-gray-500">{n(Number(r.orders))} {t('admin.bi.orderCountSuffix')}</span>
                  <span className="text-sm font-medium text-gray-900 mr-auto">{n(Number(r.gmv))} {t('common.toman')}</span>
              ...
              <tbody>
                {cohorts.map((r) => (
                  <tr key={r.cohortMonth} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2">{n(Number(r.cohortSize))}</td>
                    <td className="py-2">{n(Number(r.repeatCustomers))}</td>
                    <td className="py-2">
          ...
          <Card>
                  ...
                  <div key={s.key}>
                    <div className="flex items-center justify-between text-sm mb-1">
                      ...
                      <span className="text-gray-500">
                        {n(s.count)} ({n(s.conversionFromStart)}٪)
                      </span>
                    </div>
                    ...
                    {i > 0 && s.dropOffRate > 0 && (
                      <p className="text-[11px] text-red-500 mt-0.5">
                        {n(s.dropOffRate)}٪ {t('admin.bi.dropOffSuffix')}
                      </p>
                    )}
                  </div>
                );
            ...
            {funnel.cancelled > 0 && (
              <p className="text-xs text-gray-500 mt-4 border-t pt-3">
                {n(funnel.cancelled)} {t('admin.bi.cancelledOrdersSuffix')}
              </p>
            )}
          </Card>
        </div>
