    ...
    kpis: (
      <div className="grid sm:grid-cols-4 gap-3">
        <Card>
          ...
          <p className="text-xl font-bold text-gray-800">{n(kpis.venueCount)}</p>
        </Card>
        <Card>
          <p className="text-xs text-gray-500">{t('admin.dashboard.kpiCustomerCount')}</p>
          <p className="text-xl font-bold text-gray-800">{n(kpis.customerCount)}</p>
        </Card>
        <Card>
          <p className="text-xs text-gray-500">{t('admin.dashboard.kpiOrdersTodayWeekMonth')}</p>
          <p className="text-xl font-bold text-gray-800">
            {n(kpis.ordersToday)} / {n(kpis.ordersWeek)} /{' '}
            {n(kpis.ordersMonth)}
          </p>
        </Card>
        <Card>
          <p className="text-xs text-gray-500">{t('admin.dashboard.kpiTotalCommission')}</p>
          <p className="text-xl font-bold text-primary-700">{n(kpis.totalCommission)} {t('common.toman')}</p>
        </Card>
      </div>
    ),
    gmv: (
      <Card>
        ...
        <p className="text-2xl font-bold text-gray-800">{n(kpis.gmv)} {t('common.toman')}</p>
      </Card>
    ),
    growth: (
      <Card>
        ...
        <div className="space-y-2">
          {feed.recentOrders.slice(0, 8).map((order) => (
            <div key={order.id} className="text-sm flex items-center justify-between border-b border-gray-100 pb-1">
              ...
              <span className="text-gray-500">{n(Number(order.totalAmount))} {t('common.toman')}</span>
            </div>
          ))}
        </div>
      </Card>
