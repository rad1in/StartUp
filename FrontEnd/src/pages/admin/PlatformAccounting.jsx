...
function PnLBar({ revenue, costs, t, n }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs font-medium text-green-700 w-32 text-right">{n(Number(revenue))}</span>
      </div>
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs font-medium text-red-600 w-32 text-right">{n(Number(costs))}</span>
      </div>
    </div>
  );
}
          ...
          {pnl && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <Card><p className="text-xs text-gray-500">{t('admin.platformAccounting.commissionRevenue')}</p><p className="text-lg font-bold text-green-700">{n(Number(pnl.commissionRevenue))}</p></Card>
                <Card><p className="text-xs text-gray-500">{t('admin.platformAccounting.totalCosts')}</p><p className="text-lg font-bold text-red-600">{n(Number(pnl.totalCosts))}</p></Card>
                <Card><p className="text-xs text-gray-500">{t('admin.platformAccounting.netProfit')}</p><p className={`text-lg font-bold ${pnl.netProfit >= 0 ? 'text-primary-700' : 'text-red-600'}`}>{n(Number(pnl.netProfit))}</p></Card>
              ...
              <Card>
                ...
                <PnLBar revenue={pnl.commissionRevenue} costs={pnl.totalCosts} t={t} n={n} />
              </Card>
                  ...
                  <div className="space-y-2">
                    {Object.entries(pnl.costBreakdown).map(([cat, amount]) => (
                      <div key={cat} className="flex items-center justify-between text-sm py-1 border-b last:border-0">
                        ...
                        <span className="font-medium">{n(Number(amount))} {t('common.toman')}</span>
                      </div>
                    ))}
                  </div>
                </Card>
      ...
      {activeTab === 'yearly' && yearlyPnL && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Card><p className="text-xs text-gray-500">{t('admin.platformAccounting.yearlyRevenue')}</p><p className="text-lg font-bold text-green-700">{n(Number(yearlyPnL.totals.commissionRevenue))}</p></Card>
            <Card><p className="text-xs text-gray-500">{t('admin.platformAccounting.yearlyCosts')}</p><p className="text-lg font-bold text-red-600">{n(Number(yearlyPnL.totals.totalCosts))}</p></Card>
            <Card><p className="text-xs text-gray-500">{t('admin.platformAccounting.yearlyNetProfit')}</p><p className={`text-lg font-bold ${yearlyPnL.totals.netProfit >= 0 ? 'text-primary-700' : 'text-red-600'}`}>{n(Number(yearlyPnL.totals.netProfit))}</p></Card>
          </div>
              ...
              <tbody>
                {yearlyPnL.months.map((m) => (
                  <tr key={m.month} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 text-green-700">{n(Number(m.commissionRevenue))}</td>
                    <td className="py-2 text-red-600">{n(Number(m.totalCosts))}</td>
                    <td className={`py-2 font-medium ${m.netProfit >= 0 ? 'text-primary-700' : 'text-red-600'}`}>{n(Number(m.netProfit))}</td>
              ...
              <tbody>
                {commission.map((r, i) => (
                  <tr key={i} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2">{n(Number(r.orders))}</td>
                    <td className="py-2">{n(Number(r.gmv))}</td>
                    <td className="py-2 text-green-700 font-medium">{n(Number(r.commission))}</td>
                    <td className="py-2">{n(Number(r.avgCommissionPerOrder))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="space-y-2">
            {costs.map((c) => (
              <Card key={c.id} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <span className="font-medium text-gray-900">{n(Number(c.amount))} {t('common.toman')}</span>
