...
function buildFeatureRows(t, n) {
  return [
    { key: 'commissionRate', label: t('pricing.commissionRate'), type: 'root', format: (v) => `${(v * 100).toFixed(0)}٪` },
    { key: 'monthlyPrice',   label: t('pricing.monthlyPriceLabel'), type: 'root', format: (v) => v === 0 ? t('pricing.free') : n(Number(v)) },
...
export default function Pricing() {
  ...
  const FEATURE_ROWS = buildFeatureRows(t, n);
  function formatPrice(amount) {
    if (!amount || amount === 0) return t('pricing.free');
    return `${n(Number(amount))} ${t('pricing.toman')}`;
  }
  const { user } = useAuth();
    ...
    <div className="max-w-5xl mx-auto space-y-10">
                ...
                {plan.monthlyPrice > 0 && (
                  <div className="mt-1">
                    ...
                    {savings > 0 && billing === 'yearly' && (
                      <p className="text-xs text-green-600 mt-0.5">{t('pricing.savingsPrefix')} {n(Number(savings))} {t('pricing.toman')} {t('pricing.perYear')}</p>
                    )}
                  </div>
                )}
              </div>
      ...
      {trialStatus?.isActive && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
          ...
          <p className="text-sm text-green-700 mt-1">{n(trialStatus.daysRemaining)} {t('pricing.daysRemaining')} — {t('pricing.trialRevenue')}: {n(Number(trialStatus.revenue))} {t('pricing.toman')}</p>
          {trialStatus.revenue < trialStatus.revenueThreshold && (
            <p className="text-xs text-green-600 mt-1">
              {t('pricing.reachRevenuePrefix')} {n(trialStatus.effectiveDays === trialStatus.daysRemaining + (7 - 5) ? 7 : 5)} {t('pricing.reachRevenueMiddle')} {n(Number(trialStatus.revenueThreshold))} {t('pricing.reachRevenueSuffix')}
            </p>
          )}
        </div>
      )}
                ...
                <div key={est.tier} className={`rounded-xl p-4 border ${isBest ? 'border-primary-400 bg-primary-50' : 'border-gray-200'}`}>
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span>{n(Number(est.commission))}</span>
                    </div>
                    {est.fixed > 0 && (
                      <div className="flex justify-between">
                        <span className="text-gray-500">{t('pricing.monthlySubscription')}</span>
                        <span>{n(Number(est.fixed))}</span>
                      </div>
                    )}
                    <div className="flex justify-between font-bold border-t pt-1 mt-1">
                      <span>{t('pricing.monthlyTotal')}</span>
                      <span className={isBest ? 'text-primary-700' : ''}>{n(Number(est.total))}</span>
                    </div>
                    {billing === 'yearly' && (
                      <>
                        <div className="flex justify-between text-xs text-gray-500 mt-1">
                          <span>{t('pricing.yearlyTotal')}</span>
                          <span>{n(Number(est.yearlyTotal))}</span>
                        </div>
                        {est.yearlyPrice > 0 && (
                          <p className="text-xs text-green-600 mt-1">
                            {t('pricing.savingsPrefix')} {n(Number(est.total * 12 - est.yearlyTotal))} {t('pricing.savingsVsMonthlySuffix')}
                          </p>
                        )}
                      </>
                    )}
