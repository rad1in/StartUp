...
export default function Cafes() {
  ...
  return (
    <div className="space-y-8">
      <header className="text-center pt-6 pb-2 relative animate-rise-in">
        <div className="pointer-events-none absolute -z-10 left-1/2 -translate-x-1/2 -top-2 w-[min(26rem,90vw)] h-40 rounded-full bg-accent-500/15 blur-[80px]" />
        <p className="inline-flex items-center gap-2 glass-clear text-ink/70 text-xs font-bold px-3.5 py-1.5 rounded-full mb-4 tabular-nums">
          <span className="w-1.5 h-1.5 rounded-full bg-accent-500" />
          {n(visibleVenues.length)} {t('cafes.countLabel')}
        </p>
        <h1 className="heading-display text-display-sm sm:text-display text-ink text-balance">
          {t('cafes.titlePrefix')} <span className="text-gradient-accent">{activeCity || t('cafes.yourCity')}</span>
        </h1>
        <p className="text-ink/60 text-lg mt-3.5">{t('cafes.subtitle')}</p>
      </header>
        ...
        <div className="grid sm:grid-cols-3 gap-5">
          ...
          <div>
            <label className="block text-sm font-bold text-ink mb-2">
              {t('cafes.maxDistanceLabel')} {n(maxDistance)} {t('cafes.meters')}
            </label>
            <input
              type="range"
              min={200}
