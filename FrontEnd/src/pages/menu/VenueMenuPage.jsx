        ...
        <div className="mb-4 rounded-2xl bg-gradient-to-l from-accent-500 to-accent-600 text-white px-4 py-3 flex items-center gap-3 shadow-accent-glow animate-fade-up">
          ...
          <div>
            ...
            <p className="text-xs text-white/85 mt-0.5">
              {n(happyHour.discountPercent)}٪ {t('menu.happyHourAutoDiscount')}
            </p>
          </div>
        </div>
      )}
          ...
          <div className="flex gap-2.5 overflow-x-auto no-scrollbar pb-1">
            {recommendations.map((item) => (
              <div key={item.id} className="glass rounded-2xl p-3 shrink-0 w-40">
                ...
                <p className="text-xs text-ink/50 mt-0.5">{n(item.price)} {t('menu.toman')}</p>
                <button
                  type="button"
            ...
            {combos.map((combo) => (
              <Card key={combo.id} className="flex items-center justify-between gap-4 border-accent-200/60 bg-accent-50/40">
                <div>
                  ...
                  <p className="text-accent-700 font-extrabold mt-2">{n(combo.price)} <span className="text-xs font-medium text-ink/40">{t('menu.toman')}</span></p>
                </div>
        ...
        <section key={category.id} id={`menu-sec-${category.id}`} className="mb-6 scroll-mt-32">
          <h2 className="flex items-center gap-2 font-extrabold text-ink mb-3">
            ...
            <span className="text-xs font-medium text-ink/35">
              {n((itemsByCategory.get(category.id) || []).length)} {t('menu.itemsCountSuffix')}
            </span>
          </h2>
          <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div className="space-y-3 mb-3">
            ...
            {couponPreview && (
              <p className="text-xs text-ink font-medium">
                {t('menu.couponApplied')}: {n(couponPreview.discountAmount)} {t('menu.toman')} {t('menu.discountLabel')}
              </p>
            )}
            ...
            {user?.role === 'CUSTOMER' && loyaltyBalance > 0 && (
              <div>
                <label className="text-xs text-gray-600">
                  {t('menu.useLoyaltyPoints')} ({t('menu.balanceLabel')} {loyaltyBalance} {t('menu.pointsLabel')}، {t('menu.perPointValue')} {n(1000)} {t('menu.toman')})
                </label>
                <input
                  type="number"
                  min={0}
            ...
            {user?.role === 'CUSTOMER' && walletBalance > 0 && (
              <div>
                <label className="text-xs text-gray-600">
                  {t('menu.payFromWallet')} ({t('menu.walletBalanceLabel')} {n(walletBalance)} {t('menu.toman')})
                </label>
                <input
                  type="number"
                  min={0}
                ...
                {walletAmount > 0 && (
                  <p className="text-xs text-green-600 mt-1">
                    {n(walletAmount)} {t('menu.toman')} {t('menu.fromWallet')} — {t('menu.remainingFromGateway')}: {n(Math.max(0, subtotal - totalDiscount - walletAmount))} {t('menu.toman')}
                  </p>
                )}
              </div>
            )}
            ...
            {user?.role === 'CUSTOMER' && estimatedLoyaltyPoints > 0 && (
              <p className="flex items-center gap-1.5 text-xs font-bold text-accent-800 bg-accent-50 border border-accent-200/70 rounded-xl px-3 py-2">
                <Gift size={13} />
                {t('menu.earnPointsPrefix')} {n(estimatedLoyaltyPoints)} {t('menu.earnPointsSuffix')}
              </p>
            )}
          </div>
        )}
              ...
              <Card key={plan.id} className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {plan.menuItemName} — {plan.totalCredits} {t('menu.unitsWord')} {t('menu.forPriceWord')} {n(plan.price)} {t('menu.toman')}
                  </p>
                </div>
                {user?.role === 'CUSTOMER' && (
