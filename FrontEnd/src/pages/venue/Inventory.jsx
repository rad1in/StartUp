          ...
          <tbody>
            {margins.map((m) => (
              <tr key={m.id} className="border-b last:border-0">
                ...
                <td className="px-4 py-2.5 text-gray-600">{n(m.price)} {t('venue.inventory.tomanShort')}</td>
                <td className="px-4 py-2.5 text-gray-600">{m.cost === null ? '—' : `${n(Math.round(m.cost))} ${t('venue.inventory.tomanShort')}`}</td>
                <td className={`px-4 py-2.5 font-medium ${m.margin === null ? 'text-gray-400' : m.margin > 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {m.margin === null ? '—' : `${n(Math.round(m.margin))} ${t('venue.inventory.tomanShort')}`}
                </td>
                <td className={`px-4 py-2.5 font-medium ${m.marginPercent === null ? 'text-gray-400' : m.marginPercent > 30 ? 'text-green-600' : m.marginPercent > 0 ? 'text-yellow-600' : 'text-red-600'}`}>
                  {m.marginPercent === null ? '—' : `${n(m.marginPercent)}٪`}
                </td>
              </tr>
            ))}
          </tbody>
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {t('venue.inventory.stockLabel')}: <span className={`font-medium ${isLow ? 'text-red-600' : 'text-gray-900'}`}>{n(Number(m.currentStock))}</span> {m.unit}
                    {' — '}{t('venue.inventory.thresholdLabel')}: {n(Number(m.reorderThreshold))} {m.unit}
                  </p>
        ...
        {recipes.map((r) => (
          <div key={r.id} className="px-4 py-3 flex items-center justify-between bg-white">
            <div>
              ...
              <p className="text-xs text-gray-500">{n(Number(r.quantity))} {r.unit} {t('venue.inventory.fromMaterial')} {r.materialName}</p>
            </div>
    ...
    <div className="space-y-4">
      <Card>
        ...
        <form onSubmit={save} className="grid sm:grid-cols-2 gap-3">
          <select className="border border-gray-300 rounded-lg px-3 py-2 text-sm" value={form.rawMaterialId} onChange={(e) => setForm({ ...form, rawMaterialId: e.target.value })} required>
            <option value="">{t('venue.inventory.selectMaterial')}</option>
            {materials.map((m) => <option key={m.id} value={m.id}>{m.name} — {t('venue.inventory.stockLabel')}: {n(Number(m.currentStock))} {m.unit}</option>)}
          </select>
          ...
          <div key={a.id} className="px-4 py-3 flex items-center justify-between bg-white">
            <div>
              <p className="text-sm font-medium text-gray-800">
                {a.materialName} — <span className={Number(a.delta) > 0 ? 'text-green-600' : 'text-red-600'}>{Number(a.delta) > 0 ? '+' : ''}{n(Number(a.delta))} {a.unit}</span>
              </p>
              <p className="text-xs text-gray-500">{REASON_LABELS[a.reason] || a.reason} {a.performedByName && `— ${a.performedByName}`} — {date(a.createdAt)}</p>
          ...
          <Card key={po.id}>
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                ...
                <p className="text-xs text-gray-500">{n(Number(po.totalCost))} {t('common.toman')} — {dateShort(po.createdAt)}</p>
              </div>
              <div className="flex gap-2">
            ...
            {po.items?.length > 0 && (
              <ul className="mt-2 text-xs text-gray-500 space-y-0.5">
                {po.items.map((item) => (
                  <li key={item.id}>{item.materialName}: {n(Number(item.quantity))} {item.unit} — {t('venue.inventory.perUnit')} {n(Number(item.costPerUnit))} {t('common.toman')}</li>
                ))}
              </ul>
            )}
          </Card>
