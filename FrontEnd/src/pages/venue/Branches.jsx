...
function BranchComparisonChart({ branches }) {
  ...
  const METRIC_DEFS = [
    { key: 'totalRevenue', label: t('venue.branches.metricRevenue'), color: '#1D1B16', format: (v) => `${n(Number(v))} ${t('common.toman')}` },
    { key: 'totalOrders', label: t('venue.branches.metricOrderCount'), color: '#6C6960', format: (v) => n(Number(v)) },
    { key: 'avgOrderValue', label: t('venue.branches.metricAvgOrderValue'), color: '#A9A496', format: (v) => `${n(Math.round(v))} ${t('common.toman')}` },
    { key: 'uniqueCustomers', label: t('venue.branches.metricUniqueCustomers'), color: '#C7A24A', format: (v) => n(Number(v)) },
  ];
          ...
          {selectedBrand === brand.id && report && (
            <div className="mt-4 pt-4 border-t space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: t('venue.branches.kpiTotalOrders'), value: n(report.totals.totalOrders) },
                  { label: t('venue.branches.kpiTotalRevenue'), value: `${n(Number(report.totals.totalRevenue))} ${t('common.toman')}` },
                  { label: t('venue.branches.kpiTotalCommission'), value: `${n(Number(report.totals.totalCommission))} ${t('common.toman')}` },
                  { label: t('venue.branches.metricUniqueCustomers'), value: n(report.totals.uniqueCustomers) },
                ].map((kpi) => (
