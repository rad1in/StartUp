      ...
      {summary && (
        <div className="grid sm:grid-cols-4 gap-3">
          ...
          <Card>
            ...
            <p className="text-xl font-bold text-gray-800">{n(summary.totalRevenue)}</p>
          </Card>
          <Card>
            <p className="text-xs text-gray-500">{t('venue.accounting.platformCommission')}</p>
            <p className="text-xl font-bold text-gray-800">{n(summary.totalCommission)}</p>
          </Card>
          <Card>
            <p className="text-xs text-gray-500">{t('venue.accounting.netRevenue')}</p>
            <p className="text-xl font-bold text-primary-700">{n(summary.netRevenue)}</p>
          </Card>
        </div>
      )}
            ...
            <div key={r.id} className="flex items-center justify-between text-sm bg-gray-50 rounded-lg px-3 py-2">
              <div>
                <p className="text-gray-700">
                  {dateShort(r.periodStart)} {t('venue.accounting.dateRangeTo')} {dateShort(r.periodEnd)} (
                  {r.frequency === 'MONTHLY' ? t('venue.accounting.monthly') : t('venue.accounting.weekly')})
                </p>
                <p className="text-xs text-gray-500 mt-0.5">
                  {n(r.summary.orderCount)} {t('venue.accounting.ordersSuffix')} — {t('venue.accounting.netLabel')} {n(r.summary.netRevenue)} {t('common.toman')}
                </p>
              </div>
            </div>
          ))}
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          {byCategory.map((c) => (
            <Card key={c.category}>
              ...
              <p className="font-bold text-gray-800">{n(c.revenue)} {t('common.toman')}</p>
            </Card>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2">
          {expenses.map((exp) => (
            <Card key={exp.id} className="flex items-center justify-between">
              ...
              <p className="font-bold text-gray-800">{n(Number(exp.amount))} {t('common.toman')}</p>
            </Card>
          ))}
        </div>
      </div>
            ...
            <Card key={payout.id} className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-800">
                  {dateShort(payout.periodStart)} {t('venue.accounting.dateRangeTo')}{' '}
                  {dateShort(payout.periodEnd)}
                </p>
                <p className="font-bold text-gray-800 mt-1">{n(Number(payout.amount))} {t('common.toman')}</p>
              </div>
              <span
