...
const config = {
  ...
  // Signs the self-hosted captcha challenge tokens (see lib/selfCaptcha.js) —
  // stateless by design (no DB row per challenge), so this key is what makes
  // a token unforgeable. Separate from JWT_SECRET for the same reason as above.
  captchaSecret: process.env.CAPTCHA_SECRET || 'default-captcha-secret-change-me',
