...
async function getStatus() {
  ...
  return {
    ...
    captcha: {
      // Self-hosted (see lib/selfCaptcha.js) — no key to configure, just on/off.
    ...
    analytics: {
      ...
      // HTML) — echoed back in full, unlike an actual secret key.
...
async function updateCaptchaSettings({ enabled }, actingUserId) {
  if (enabled !== undefined) await setSetting('captcha.enabled', Boolean(enabled));
  await logActivity(null, actingUserId, 'INTEGRATION_CAPTCHA_UPDATED', 'PlatformSetting', null, { enabled });
  return getStatus();
}
