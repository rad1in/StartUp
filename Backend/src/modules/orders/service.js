...
const REVIEW_REMINDER_DELAY_MINUTES = 30;

// Polled once a minute from server.js (same pattern as the other due-item
// jobs there) rather than a per-order setTimeout — survives server restarts
// and never piles up in-process timers. Only fires once per order
// (reviewReminderSentAt gate) and skips anyone who already reviewed.
async function processDueReviewReminders() {
  const [orders] = await pool.query(
    `SELECT o.id, o.customerId, v.name AS venueName
     FROM \`Order\` o
     ...
     WHERE o.status = 'SERVED'
       ...
       AND o.updatedAt <= DATE_SUB(NOW(), INTERVAL ? MINUTE)
       AND r.id IS NULL
     LIMIT 50`,
    [REVIEW_REMINDER_DELAY_MINUTES]
  );
  ...
  for (const order of orders) {
    await createNotification(
      ...
      'SYSTEM',
      'نظرت رو بگو',
      `سفارشت از ${order.venueName} چطور بود؟ چند ثانیه وقت بذار و نظرت رو ثبت کن.`,
      { orderId: order.id }
    );
    await pool.query('UPDATE `Order` SET reviewReminderSentAt = NOW() WHERE id = ?', [order.id]);
  }
}
