...
export default function HomeScreen() {
  ...
  const header = (
    <View style={{ marginBottom: 22 }}>
      ...
      <T style={{ fontFamily: fonts.bold, fontSize: 12, letterSpacing: 1.4, color: colors.gold300, marginBottom: 8 }}>
        {t(greetingKey()).toUpperCase()}
      </T>
      <T style={{ fontFamily: fonts.black, fontSize: 38, lineHeight: 46, letterSpacing: -0.8 }}>{t('nearbyTitle')}</T>
      <Muted style={{ marginTop: 10, fontSize: 15, lineHeight: 22 }}>
        {t('nearbySubtitle', { radius: n(radiusMeters) })}
      </Muted>
    </View>
  );
